...
function renderProduct() {
  ...
  const content = `
    <div class="detail-hero"><button class="back-link" data-route="recipes">← Recipe book</button><div class="detail-heading"><div><span class="record-id">${e(product.id)}</span><h2>${e(product.name)}</h2><p>${e(product.type)} · ${e(product.primaryUse)}</p></div><div class="detail-actions">${/^HIGH/i.test(product.priority) ? badge('High priority','orange') : ''}${badge(product.status)}${feedbackEnabled() ? `<button class="primary-button feedback-cta" data-feedback="${e(product.key)}">✎ Tap feedback</button>` : ''}</div></div><div class="approval-banner approval-experimental"><strong>${e(product.approval)}</strong><span>Recorded formulations and tests do not imply production or shelf-life approval.</span></div></div>
