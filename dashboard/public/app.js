...
function renderProduct() {
  ...
  const testCards = tests.length ? tests.map((test) => `<button class="application-card" data-route="test/${encodeURIComponent(test.id)}"><div><span class="record-id">${e(test.id)}</span><h3>${e(test.name)}</h3><p>${e(test.category)} · ${e(testDose(test))} sauce</p></div>${badge(test.result)}</button>`).join('') : `<div class="empty-state compact"><strong>Not yet tested</strong><p>No drink application has been recorded for this product.</p></div>`;
