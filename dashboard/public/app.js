...
function shortBatchName(batch) { return /PUR[ÉE]E|PUREE/i.test(`${batch?.product} ${batch?.version}`) ? 'Purée' : /SYRUP/i.test(`${batch?.product} ${batch?.version}`) ? 'Syrup' : batch?.version || 'Batch'; }
function feedbackTargetName(id) {
  const batch = state.data?.batches.find((item) => item.id === id);
  if (batch) return `${shortBatchName(batch)} · ${batch.version}`;
  const test = state.data?.tests.find((item) => item.id === id);
  return test ? `${test.name} · ${test.id}` : id || 'Product overall';
}
function batchFeedbackButton(batch) { return feedbackEnabled() ? `<button class="quick-target-feedback" data-feedback="${e(batch.productId)}" data-target="${e(batch.id)}">✎ Feedback: ${e(shortBatchName(batch))}</button>` : ''; }
...
function feedbackEnabled() { return !hostedReadOnly() || Boolean(state.data?.hostedFeedbackEnabled); }
...
function shell(content, pageTitle, eyebrow, subtitle = '') {
  ...
  return `<div class="shell">
    ...
    <main class="main"><header class="topbar ${detailPage ? 'compact-topbar' : ''}"><div><div class="eyebrow">${e(eyebrow)}</div>${detailPage ? '' : `<h1>${e(pageTitle)}</h1>${subtitle ? `<p>${e(subtitle)}</p>` : ''}`}</div><div class="top-actions">${hosted ? `<span class="hosted-pill">Phone access · ${feedbackEnabled() ? 'feedback on' : 'viewing mode'}</span>` : '<button class="quiet-button" data-note>＋ Control Room Note</button><button class="primary-button" data-feedback>＋ Add Feedback</button>'}</div></header>${hosted ? `<div class="hosted-banner"><div><strong>Live recipes from GitHub${feedbackEnabled() ? ' · feedback saves here' : ''}.</strong><span>${feedbackEnabled() ? 'Tap the feedback button on a recipe, choose observations, and save—typing is optional.' : 'Open briefs, research decisions, sourced recipes and shopping details from this phone view.'}</span></div><button class="text-button" data-route="home">Open priorities</button></div>` : ''}${content}</main>
...
function ingredientTable(record) {
  ...
  const keyFor = (pattern) => headers.find((key) => pattern.test(key));
  const ingredientKey = keyFor(/^ingredient$/i) || headers[0];
  const amountKey = keyFor(/^(planned )?amount$|quantity/i);
  const unitKey = keyFor(/^unit$/i);
  const compact = rows.map((row) => {
    const ingredient = row[ingredientKey] || 'Ingredient';
    const measure = [amountKey ? row[amountKey] : '', unitKey ? row[unitKey] : ''].filter((value) => value && value !== 'UNKNOWN').join(' ');
    return `<div class="ingredient-line"><span>${e(ingredient)}</span>${measure ? `<strong>${e(measure)}</strong>` : ''}</div>`;
  }).join('');
  return `<div class="ingredient-desktop">${table(useful.map((key) => ({key,label:key})), rows)}</div><div class="ingredient-mobile" aria-label="Ingredients">${compact}</div>`;
}
...
function renderProduct() {
  ...
  const content = `
    <div class="detail-hero"><button class="back-link" data-route="recipes">← Recipe book</button><div class="detail-heading"><div><span class="record-id">${e(product.id)}</span><h2>${e(product.name)}</h2><p>${e(product.type)} · ${e(product.primaryUse)}</p></div><div class="detail-actions">${/^HIGH/i.test(product.priority) ? badge('High priority','orange') : ''}${badge(product.status)}${feedbackEnabled() ? `<button class="primary-button feedback-cta" data-feedback="${e(product.key)}">✎ Overall feedback</button>` : ''}</div></div><div class="approval-banner approval-experimental"><strong>${e(product.approval)}</strong><span>Recorded formulations and tests do not imply production or shelf-life approval.</span></div></div>
    <nav class="anchor-nav"><button type="button" data-scroll="overview">Overview</button><button type="button" data-scroll="recipe">Recipe</button><button type="button" data-scroll="provenance">Source & Provenance</button><button type="button" data-scroll="history">Development History</button><button type="button" data-scroll="tests">Test Results</button><button type="button" data-scroll="applications">Applications</button></nav>
...
function renderFeedback() {
  ...
  const cards = feedback.length ? feedback.map((item)=>`<article class="feedback-card"><div class="feedback-meta"><span class="avatar">${e((item.authorName || item.author || '?')[0])}</span><div><strong>${e(item.authorName || item.author)}</strong><small>${e(productName(item.productId))} · ${e(item.context)} · ${formatDate(item.timestamp,true)}</small></div>${item.targetId ? `<span class="record-id feedback-target">${e(feedbackTargetName(item.targetId))}</span>` : '<span class="record-id feedback-target">Product overall</span>'}</div>${item.tags?.length ? `<div class="tag-row">${item.tags.map((tag)=>`<span>${e(tag)}</span>`).join('')}</div>`:''}${item.note ? `<blockquote>${e(item.note)}</blockquote>`:''}${Object.keys(item.ratings||{}).length ? `<div class="rating-summary">${Object.entries(item.ratings).map(([key,value])=>`<span>${e(titleCase(key))} <strong>${value}/5</strong></span>`).join('')}</div>`:''}</article>`).join('') : `<div class="empty-state"><div class="empty-icon">✎</div><strong>No feedback recorded yet</strong><p>Add a tasting, service, or visual observation. It will be stored as an append-only repository record.</p><button class="primary-button" data-feedback>＋ Add first feedback</button></div>`;
  const content = `<div class="feedback-intro"><div><strong>${feedbackEnabled() ? 'Fast enough for the café' : 'Repository feedback record'}</strong><p>${feedbackEnabled() ? 'Choose one or more observations and save. Typing is optional.' : 'This hosted mirror shows committed feedback but cannot save new entries.'}</p></div>${feedbackEnabled() ? '<button class="primary-button large" data-feedback>✎ Tap feedback</button>' : ''}</div><section class="feedback-list">${cards}</section>`;
...
function feedbackModal() {
  ...
  const target = state.modal.targetId || '';
  const targets = ['',...selected.batchIds,...selected.testIds].map((id)=>`<option value="${e(id)}" ${target===id?'selected':''}>${e(feedbackTargetName(id))}</option>`).join('');
  ...
  return `<div class="modal-backdrop" data-close><section class="modal-sheet feedback-sheet" role="dialog" aria-modal="true" aria-labelledby="feedback-title" data-modal-body><header><div><span class="kicker">Tap-only café check</span><h2 id="feedback-title">Feedback: ${e(feedbackTargetName(target))}</h2><p class="modal-lead">This observation will apply only to the selected preparation or test. No typing is required.</p></div><button class="close-button" data-close aria-label="Close">×</button></header><form id="feedback-form"><div class="form-grid feedback-basics"><label>Product<select name="productId">${state.data.products.map((p)=>`<option value="${e(p.productId||p.key)}" ${(p.key===selected.key)?'selected':''}>${e(p.name)}</option>`).join('')}</select></label><label>Preparation or test<select name="targetId">${targets}</select></label><label>Tester<select name="author"><option>Andreas</option><option>Huong</option><option>Other</option></select></label><label>Tested in<select name="context">${['Sauce/Cordial itself','Matcha','Coffee','Tea','Zero-proof','Cocktail','Service','Visual','Other'].map((v)=>`<option>${v}</option>`).join('')}</select></label></div><input type="hidden" name="authorName" value=""><fieldset class="quick-feedback-fieldset"><legend>What did you notice? <small>Tap all that apply</small></legend><div class="tag-picker quick-tags">${[...baseTags,...productTags].map((tag)=>`<label><input type="checkbox" name="tags" value="${e(tag)}"><span>${e(tag)}</span></label>`).join('')}</div></fieldset><details class="optional-feedback"><summary>Optional ratings or note</summary><fieldset><legend>Ratings <small>1–5</small></legend><div class="rating-grid">${ratingFields.map((key)=>`<label><span>${titleCase(key)}</span><select name="rating:${e(key)}"><option value="">—</option>${[1,2,3,4,5].map((v)=>`<option value="${v}">${v}</option>`).join('')}</select></label>`).join('')}</div></fieldset><label>Note<textarea name="note" rows="3" placeholder="Optional detail"></textarea></label></details><div class="form-message" aria-live="polite"></div><footer><button type="button" class="quiet-button" data-close>Cancel</button><button type="submit" class="primary-button">Save feedback</button></footer></form></section></div>`;
}
...
function bind() {
  ...
  document.querySelectorAll('[data-scroll]').forEach((el)=>el.addEventListener('click',()=>document.querySelector(`#${el.dataset.scroll}`)?.scrollIntoView({behavior:'smooth',block:'start'})));
  ...
  document.querySelector('#feedback-form [name="targetId"]')?.addEventListener('change',(event)=>{state.modal.targetId=event.target.value;render();});
...
async function submitFeedback(event) {
  ...
  try{message.textContent='Saving…';await api('/api/feedback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});await reloadData();state.modal=null;showToast(hostedReadOnly() ? 'Feedback saved from this phone' : 'Feedback added to the repository');render();}catch(error){message.textContent=error.message;}
}
