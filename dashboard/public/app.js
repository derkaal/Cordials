...
function shell(content, pageTitle, eyebrow, subtitle = '') {
  ...
  return `<div class="shell">
    ...
    <main class="main"><header class="topbar ${detailPage ? 'compact-topbar' : ''}"><div><div class="eyebrow">${e(eyebrow)}</div>${detailPage ? '' : `<h1>${e(pageTitle)}</h1>${subtitle ? `<p>${e(subtitle)}</p>` : ''}`}</div><div class="top-actions">${hosted ? '<span class="hosted-pill">Phone view · read-only</span>' : '<button class="quiet-button" data-note>＋ Control Room Note</button><button class="primary-button" data-feedback>＋ Add Feedback</button>'}</div></header>${hosted ? '<div class="hosted-banner"><strong>Synced from GitHub main</strong><span>This phone-accessible view is read-only. Add notes and change shopping status in the local dashboard so the repository remains the single source of truth.</span></div>' : ''}${content}</main>
...
function renderProduct() {
  ...
  const batch = linkedBatches.find((item) => !/SUPERSEDED|WITHDRAWN/i.test(item.status)) || linkedBatches[0];
  ...
  const content = `
    ...
    <section id="recipe" class="detail-section"><div class="section-title"><div><span class="kicker">Formulations</span><h2>${cordialRef ? 'Two distinct preparation options' : 'Current formulation'}</h2></div></div>${batchPanel}${cordialPanel}</section>
...
function renderShopping() {
  ...
  const groupHtml = groups.map((group) => {
    ...
    return `<section class="shopping-group"><div class="section-title"><div><span class="kicker">${group === 'Need now' ? 'Required for authorised work' : group === 'Soon' ? 'Upcoming work' : 'Controls and alternatives'}</span><h2>${group}</h2></div><span>${items.length} items</span></div><div class="shopping-list">${items.map((item)=>`<article class="shopping-item ${['Bought','Available','Not needed'].includes(item.state)?'resolved':''}"><select class="state-select state-${e(item.state.toLowerCase().replace(' ', '-'))}" data-shopping="${e(item.itemKey)}" aria-label="Shopping state for ${e(item.item)}" ${hostedReadOnly() ? 'disabled title="Read-only hosted mirror"' : ''}>${['Need','Ordered','Bought','Available','Not needed'].map((value)=>`<option ${item.state===value?'selected':''}>${value}</option>`).join('')}</select><div><div class="shopping-title"><h3>${e(item.item)}</h3>${item.priority ? badge(item.priority,item.group==='Need now'?'orange':'neutral'):''}</div>${item.selection ? `<p>${e(item.selection)}</p>`:''}<small><strong>Needed for:</strong> ${e(item.productLabel || 'Batch 001')} · ${e(item.reason)}</small>${item.sourceStatus ? `<small>${e(item.sourceStatus)}</small>`:''}</div>${item.url ? `<a class="purchase-link" href="${e(item.url)}" target="_blank" rel="noreferrer">Purchase source ↗</a>`:''}</article>`).join('')}</div></section>`;
  }).join('');
