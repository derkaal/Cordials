...
The local dashboard remains the writable working interface. A private, phone-accessible ChatGPT Site provides a read-only mirror that follows the generated dashboard snapshot on the public GitHub `main` branch; this keeps repository records authoritative and avoids a second writable data store.
