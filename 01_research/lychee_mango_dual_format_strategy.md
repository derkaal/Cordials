# Lychee and Mango — Dual-Format Development Strategy
...
## Professional cordial sources selected

### Lychee

`PR-0022` is the preferred first cordial lineage. Barney Toy's published Clipper formulation uses one 560 g can of lychees, 150 g caster sugar, 10 g citric acid, and 1 g sea salt, followed by blending and cold filtration. It is a zero-proof component with a sourced 35 ml highball application.
...
Execution is not yet authorised. The user's can label and actual size must be recorded, and the source's unresolved treatment of canning liquid must remain visible. If the can is not the source-stated 560 g size, a new whole-gram scaled adaptation must be written before testing.

### Mango

`PR-0021` is the preferred first cordial lineage. Michal Dojčák's professional method treats 400 g mango juice with 8 g each Töufood Pectinäse and Invertäse in a sealed bag for 24 hours under refrigeration, strains it, adds sugar to a source-stated 50 Brix endpoint, and combines the clarified syrup in equal parts with Citrus Blend diluted 1:9.

Why it fits:

- clarification is explicitly linked to improved carbonation behaviour;
- the source names fizzes, Collins, sours, and whisky highballs;
- green tea is explicitly allowed as the liquid used to dilute the acid blend;
- the already ordered refractometer measures the source's 50 Brix endpoint;
- the fixed starting quantities are whole grams.

Execution is not yet authorised. The source starts with mango juice, not Boiron purée; using the ordered purée would be a disclosed adaptation. The named enzymes, proprietary Citrus Blend, German delivery, and a liquid-compatible sealed-bag process must be resolved first. The CASO VC10 must not be assumed suitable for vacuuming an unfrozen liquid solely because it can heat-seal bags.
...
| Is it suitable for carbonation? | Not a primary purpose | Required comparison |
| Does it preserve recognisable fruit with matcha? | Required | Required |
| Does it work in tea? | Later option | Required comparison |
| Does it work in a cocktail/highball? | Optional | Required comparison |

No numerical success threshold is set before control data exist. First evaluations use paired comparative language against a suitable commercial control: `BETTER`, `NO MATERIAL DIFFERENCE`, `WORSE`, or `INCOMPARABLE`, with reasons recorded.
