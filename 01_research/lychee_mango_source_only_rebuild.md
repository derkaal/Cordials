# PB-002 / PB-003 — Source-Only Professional Rebuild

Date: 2026-08-31

Scope note (2026-09-03): this record governs the glass-cling purée-preparation tracks only. The separate cordial tracks are documented in `lychee_mango_dual_format_strategy.md`; neither track supersedes the other.

## Governing constraint

Active recipe, batch, and application formulations must be exact sourced recipes or explicit adaptations of sourced recipes from commercially active/professional sources. The discarded V01 lychee and mango plans were unexecuted original formulations and are not retained as development candidates.

## Professional candidate comparison

| Record | Professional relevance | Completeness | Fit to glass-cling purée preparation | Decision |
|---|---|---|---|---|
| PR-0019 — Boiron Lychee Confit / Compote | Primary professional manufacturer chart; fruit-specific formula | Exact grams, temperatures, sequence; yield/storage unknown | High-fruit preparation with published NH pectin and cold smoothing; cling remains untested | **SELECT FOR PB-002 LINEAGE** |
| PR-0020 — Boiron Mango Confit / Compote | Primary professional manufacturer chart; fruit-specific formula | Exact grams, temperatures, sequence; yield/storage unknown | Same professional technique with a different, source-stated mango pectin rate | **SELECT FOR PB-003 LINEAGE** |
| PR-0018 — Boiron Passion Fruit and Mango Coulis | Named professional, cold blend, squeeze-bottle use | Complete | Strong service precedent, but passion fruit materially changes flavour and acidity | Retain as secondary mango reference |
| PR-0017 — Clearspring Mango Matcha Latte | Commercially active matcha/food company | Exact mango, matcha, water and milk quantities; ice amount unstated | Direct fruit–matcha application and build-order precedent | **SELECT FOR APPLICATION LINEAGE** |
| PR-0016 — Matchaful Lucent Lychee Matcha Latte | Direct café-menu lychee–matcha precedent | Matcha mass and yield remain unknown | Supports pairing, not purée-preparation structure | Corroborating lychee application evidence |
| MONIN Le Fruit Lychee/Mango | Current commercial fruit preparations for drinks | Proprietary formula amounts unknown | Realistically purchasable squeeze/service controls | **SELECT AS CONTROLS, NOT FORMULA SOURCES** |

## Candidate decision

Use café-practical adaptations derived from the Boiron source formulas. Under the adopted bench rule, working quantities below 10 g use standard level measuring-spoon units. SRC-2026-092 reports NH pectin at an average 3.6 g per teaspoon, so the current pilots use one level 1/2 tsp and preserve the exact source calculation separately:

| Fruit | Purée | Sugar | NH pectin | Planned ingredient mass |
|---|---:|---:|---:|---:|
| Lychee | 220 g | 50 g, split 25 g + 25 g | 1 level 1/2 tsp | 270 g plus spoon dose |
| Mango | 280 g | 60 g, split 30 g + 30 g | 1 level 1/2 tsp | 340 g plus spoon dose |

The active batches prioritise café handling over calculator-perfect scaling. For lychee, the exact PR-0019 amounts at 220 g purée are 49.5 g sugar and 1.98 g pectin; V04 uses 50 g sugar and 1 level 1/2 tsp pectin. For mango, the exact PR-0020 amounts at 280 g purée are 63 g sugar and 1.96 g pectin; V04 uses 60 g and 1 level 1/2 tsp. SRC-2026-092 estimates that spoon dose at about 1.8 g, but product/packing differences remain possible. Every change is disclosed; neither batch claims an unchanged source ratio. Earlier unexecuted plans remain preserved as superseded.

For lychee, the first available-input plan replaces Boiron purée with a purée made from drained canned lychees while preserving the Boiron ratio and method. That ingredient substitution makes the batch an `ADAPTATION OF SOURCED RECIPE`. It may not run until the can label, syrup type, drained yield, and fruit Brix are recorded. A future exact-brand Boiron-input batch remains possible through `SRC-2026-062`.

For mango, the practical scale, disclosed rounding, and service-purpose change make the operational batch an `ADAPTATION OF SOURCED RECIPE`; the professional source and the numerical derivation remain visible.

## Texture boundary

NH pectin is part of PR-0019 and PR-0020. It is therefore not a separate house texture modification. Do not run an unpectinated or altered-pectin aliquot under the same batch ID. Any later change to pectin type or amount must be a new, explicit adaptation of the professional source and must show the source amount beside the changed amount.

## Application boundary

Both fruit drinks will adapt PR-0017: 2 g matcha, 60 ml water at about 80°C, 200 ml cold milk, two tablespoons of fruit component, and ice. The source's two tablespoons will be operationalised as 30 ml, with actual deposited mass recorded. The adaptations are the source-derived sauce, whole milk in place of plant milk, and application to the inner glass wall rather than the bottom. Ice quantity remains source-unstated and is controlled as a measured glass-specific method parameter rather than invented as a source quantity.

## Shelf-life boundary

The 103°C endpoint is a source-stated texture process, not proof of ambient stability or a validated refrigerated life. Flavour, texture, glass cling, squeeze service, and drink performance must pass before a freeze–thaw quality study is opened.
